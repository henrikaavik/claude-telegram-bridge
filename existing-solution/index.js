...
import { MCPQueueWatcher } from './lib/mcp-queue-watcher.js';
...
// MCP Queue Watcher (if enabled)
let mcpQueueWatcher = null;
const MCP_ENABLED = process.env.MCP_ENABLED === 'true';

if (MCP_ENABLED) {
  mcpQueueWatcher = new MCPQueueWatcher(bot, { t });
}
...
bot.on('message', async (msg) => {
  ...
  if (text && !text.startsWith('/')) {
    ...
    // Check if this is a response to an MCP question
    if (mcpQueueWatcher) {
      const handled = await mcpQueueWatcher.handleTextResponse(msg);
      if (handled) {
        await bot.sendMessage(chatId, '✅ Response sent to Claude');
        return; // MCP handled this text
      }
    }
...
bot.on('callback_query', async (query) => {
  ...
  if (mcpQueueWatcher) {
    const handled = await mcpQueueWatcher.handleUserResponse(query);
    if (handled) {
      return; // MCP handled this callback
    }
  }
...
// Start MCP Queue Watcher if enabled
if (MCP_ENABLED && mcpQueueWatcher) {
  mcpQueueWatcher.start().catch(error => {
    console.error('❌ Failed to start MCP queue watcher:', error);
  });
}
